	...
	if (target) {
		...
		if (target.interact && (!target.interactRange || distance(pony, target) < target.interactRange)) {
			...
			switch (target.interactAction) {
				...
				// baru
				case InteractAction.GiveMangkokBakso: { 
					const hold = client.pony.options!.hold;
					let mangkokBakso = hold;

					while (hold === mangkokBakso) {
						mangkokBakso = sample(entities.mangkokBaksoTypes)!;
					}
				...
				case InteractAction.GivePiringSate: {
					const hold = client.pony.options!.hold;
					let piringSate = hold;

					while (hold === piringSate) {
						piringSate = sample(entities.piringSateTypes)!;
					}
