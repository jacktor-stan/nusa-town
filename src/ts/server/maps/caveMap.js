...
var mapData = JSON.parse(fs.readFileSync(paths_1.pathTo('src', 'maps', 'cave.json'), 'utf8'));
function createCaveMap(world) {
    var map = serverMap_1.createServerMap('cave', 3 /* Cave */, 7, 7, 0 /* None */, 0 /* Public */);
    ...
    var caveDecals = [entities.caveDecal1, entities.caveDecal3, entities.caveDecal2];
    function cracksS(x, y) {
        var code = (Math.random() * 1000) % 64;
        var index1 = code & 3;
        var index2 = (code >> 2) & 3;
        var index3 = (code >> 4) & 3;
    ...
    function cracksSLeft(x, y) {
        var code = (Math.random() * 1000) % 4;
        (code & 1) && add(entities.caveDecalL(x + 0.5, y - 1));
        (code & 2) && add(entities.caveDecalL(x + 0.5, y));
    }
    function cracksSRight(x, y) {
        var code = (Math.random() * 1000) % 4;
        (code & 1) && add(entities.caveDecalR(x + 0.5, y - 1));
        (code & 2) && add(entities.caveDecalR(x + 0.5, y));
    }
    function caveSW(x, y) {
    ...
    function caveTrimLeft(x, y, h, botTrim) {
        if (botTrim === void 0) { botTrim = true; }
        if (botTrim) {
    ...
    function caveTrimRight(x, y, h, botTrim) {
        if (botTrim === void 0) { botTrim = true; }
        if (botTrim) {
    ...
    add(entities.trigger3x1(27.5, 55)).trigger = function (_, client) { return world_1.goToMap(world, client, '', 'cave'); };
    ...
    add(controllerUtils_1.createBoxOfLanterns(30.97, 49.63)).interact = function (_, client) {
    ...
    map.controllers.push(new controllers_1.FlyingCritterController(world, map, entities.bat, 2, 10, function () { return true; }, true));
    var wallController = new wallController_1.WallController(world, map, entities.stoneWalls);
    map.controllers.push(wallController);
    wallController.top = 3;
    wallController.isTall = function (x, y) {
        if (y === 10 && x >= 17 && x <= 23)
            return true;
