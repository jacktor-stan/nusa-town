...
exports.resetIslandMap = exports.createIslandMap = void 0;
var fs = require("fs");
...
var islandMapData = JSON.parse(fs.readFileSync(paths_1.pathTo('src', 'maps', 'island.json'), 'utf8'));
var islandMapTemplate;
function createIslandMap(world, instanced, template) {
    ...
    var map = (instanced && islandMapTemplate) ?
    ...
    var goto = instanced ? 'house' : 'public-house';
    var add = function (entity) { return world.addEntity(entity, map); };
    var addEntities = function (entities) { return entities.map(add); };
    add(entities.house(28, 23)).interact = function (_, client) { return world_1.goToMap(world, client, goto); };
    add(entities.triggerHouseDoor(27.40, 22.87)).trigger = function (_, client) { return world_1.goToMap(world, client, goto); };
    add(controllerUtils_1.createBoxOfLanterns(25.5, 25.5));
    var boxOfFruits = add(entities.boxFruits(20.72, 20.88));
    entityUtils_1.setEntityName(boxOfFruits, 'Box of fruits');
    var giftPile = add(entities.giftPileInteractive(37.66, 18.21));
    giftPile.interact = function (_, client) { return entityUtils_1.updateEntityOptions(client.pony, playerUtils_1.getNextToyOrExtra(client)); };
    entityUtils_1.setEntityName(giftPile, 'Toy stash');
    var types = entities.stashEntities.map(function (e) { return e.type; });
    var itemSign = add(entities.signQuest(24.41, 25.00));
    itemSign.interact = function (_, client) {
        var index = types.indexOf(client.pony.options.hold || 0);
        playerUtils_1.holdItem(client.pony, types[(index + 1) % types.length]);
    };
    entityUtils_1.setEntityName(itemSign, 'Item stash');
    var addTorch = controllerUtils_1.createAddLight(world, map, entities.torch);
    ...
    // pier
    var px = 1 / constants_1.tileWidth;
    var plankWidth = 78 / constants_1.tileWidth;
    var plankHeight = 12 / constants_1.tileHeight;
    var plankOffsets = [0, -1, 0, -2, -1, -1, 0, -2, -1, 0].map(function (x) { return x / constants_1.tileWidth; });
    ...
    add(entities.triggerBoat(45.5, 24.8)).interact = function (_, client) { return world_1.goToMap(world, client, '', 'harbor'); };
    add(controllerUtils_1.createSignWithText(43, 23.5, 'Return to land', "Hop on the boat to return to the mainland"));
    for (var y = 0; y < 10; y++) {
        var minX = y < 5 ? 0 : 1;
        var maxX = (y % 2) ? 4 : 3;
        var baseX_1 = 40 + ((y % 2) ? 0 : (plankWidth / 2)) + plankOffsets[y];
        var baseY_1 = 19 - (9 / constants_1.tileHeight);
        for (var x = minX; x < maxX; x++) {
            if ((x === minX && (y % 2)) || (x === (maxX - 1) && (y % 2))) {
                var ox = x === minX ? (18 / constants_1.tileWidth) : (-18 / constants_1.tileWidth);
                var plank = lodash_1.sample(entities.planksShort);
                add(plank(baseX_1 + ox + x * plankWidth, baseY_1 + y * plankHeight));
            }
            else {
                var plank = lodash_1.sample(entities.planks);
                add(plank(baseX_1 + x * plankWidth, baseY_1 + y * plankHeight));
            }
        }
    }
    ...
    var addWoodenFence = controllerUtils_1.createWoodenFenceMaker(world, map);
    ...
    add(entities.flower3Pickable(30.25, 24.92)).interact = function (_, _a) {
        var pony = _a.pony;
        return playerUtils_1.holdItem(pony, entities.flowerPick.type);
    };
